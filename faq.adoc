...
Ja, Astra Trident kann offline installiert werden. Siehe link:../trident-install/kubernetes-deploy.html["Erfahren Sie mehr über die Installation von Astra Trident"^].
...
Bei Bedarf können die von Astra Trident für die Installation verwendeten Manifestdateien lokal generiert und angepasst werden. Weitere Informationen finden Sie unter link:trident-install/kubernetes-customize-deploy-tridentctl.html["Die tridentctl-Installation anpassen"^].
