...
Unabhängig davon, ob die Bereitstellung manuell oder über Helm erfolgt, ist der Trident Operator eine hervorragende Möglichkeit, die Installation zu vereinfachen und Astra Trident Ressourcen dynamisch zu verwalten. Sie können sogar link:kubernetes-customize-deploy.html["Individuelle Anpassung der Trident Implementierung"] unter Verwendung der Attribute in der  `TridentOrchestrator` Custom Resource (CR).
